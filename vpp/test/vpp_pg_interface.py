...
class VppPGInterface(VppInterface):
    ...
    def get_capture(
        ...
        :param timeout: how long to wait for packets, needed if packet capture
                        is independent of packet generator (e.g. packets
                        generated by timers)
    ...
    def wait_for_packet(self, timeout, filter_out_fn=is_ipv6_misc):
        ...
        :returns: Captured packet if packet arrived within timeout
        ...
        if self._pcap_reader is None:
            if not os.path.isfile(self.out_path):
                remaining = deadline - time.time()
                if remaining <= 0 or not self._wait_for_file_inotify(
                    self.out_path, remaining
                ):
                    raise CaptureTimeoutError(
